...
describe('List Page', () => {
  ...
  it('should initially report no To-Do items', () => {
    return supertest(app)
      .get('/todo/list')
      .then(res => {
        expect(res.text).to.match(/No to-do items/i);
      });
  });
...
describe('Adding items', () => {
  ...
  it('should show items added in subsequent requests', async () => {
    ...
    expect(res.text).to.match(/Walk the dog/);
    ...
    expect (res.text).to.match(/Walk the dog[^]*Wash the dishes/);
  });
  ...
  it('should initially report items as incomplete', async () => {
    ...
    for (let i = 0; i < 2; ++i) {
      ...
      expect(cb.attr('type')).to.equal('checkbox');
